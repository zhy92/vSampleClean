const methods = {
  ...
  handleOperateButton(data) {
    let formGroupList;
    if (data.flag === "add") {
      this.dialogFormSettings.dialogFormTitle = "新增机械设备信息";
      // 加载表单组
    ...
    if (data.flag === "delete") {
      if (!this.chosedUnitData.length) {
        this.$message.warning("请选择需删除的机械设备信息!");
        return;
      }
      let unitId = this.chosedUnitData.map(item => {
        return item.fpmpBasMachineryId;
      });
      this.$confirm("您确定要删除选中的机械设备信息？", "确认", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        this.$postData(
          this.$api.removeBatchMachinery_machinerySetManage,
          {
            ids: unitId
          },
          true
    ...
    if (data.flag === "open" || data.flag === "closed") {
      ...
      if (!this.chosedUnitData.length) {
        this.$message.warning(`请选择需${quote}的机械设备信息!`);
        return;
      }
      ...
      if ([...new Set(status)].length > 1) {
        this.$message.warning("请选择相同状态的机械设备信息!");
        return;
      }
      let unitId = this.chosedUnitData.map(item => {
        return item.fpmpBasMachineryId;
      ...
      this.$postData(
        this.$api.updateStatusBatchMachinery_machinerySetManage,
        {
          ids: unitId,
          disabled: updateType
        },
  ...
  handleTableRowButton(row, btns) {
    let formGroupList;
    if (btns.type === "edit") {
      this.getSingleDetailInfo(row.fpmpBasMachineryId).then(data => {
        this.dialogFormSettings.dialogFormTitle = "编辑机械设备信息";
        // 加载表单组
    ...
    // 删除
    if (btns.type === "remove") {
      this.$confirm("确定删除当前机械设备吗?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        this.$postData(
          this.$api.removeMachinery_machinerySetManage,
          {
            fpmpBasMachineryId: row.fpmpBasMachineryId
          },
          true
        ).then(() => {
          this.$message.success("您已成功删除该机械设备!");
          this.initPageData();
        });
      });
    } // 启用禁用
    if (btns.type === "open" || btns.type === "closed") {
      ...
      this.$confirm(`您确定要${quote}该机械设备信息？`, "确认", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
  ...
  getSingleDetailInfo(id) {
    return new Promise(resolve => {
      this.$getData(this.$api.loadMachineryDetail_machinerySetManage, {
        id: id
      }).then(data => {
        resolve(data.data);
      });
