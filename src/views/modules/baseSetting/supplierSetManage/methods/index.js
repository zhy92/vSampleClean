const methods = {
  ...
  handleOperateButton(data) {
    let formGroupList;
    if (data.flag === "add") {
      this.dialogFormSettings.dialogFormTitle = "新增供应商信息";
      // 加载表单组
    ...
    if (data.flag === "delete") {
      if (!this.chosedUnitData.length) {
        this.$message.warning("请选择需删除的供应商信息!");
        return;
      }
      let unitId = this.chosedUnitData.map(item => {
        return item.fpmpBasSupplierId;
      });
      this.$confirm("您确定要删除选中的供应商信息？", "确认", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        this.$postData(
          this.$api.removeBatchSupplier_supplierSetManage,
          {
            ids: unitId
          },
          true
    ...
    if (data.flag === "open" || data.flag === "closed") {
      ...
      if (!this.chosedUnitData.length) {
        this.$message.warning(`请选择需${quote}的供应商信息!`);
        return;
      }
      ...
      if ([...new Set(status)].length > 1) {
        this.$message.warning("请选择相同状态的供应商信息!");
        return;
      }
      let unitId = this.chosedUnitData.map(item => {
        return item.fpmpBasSupplierId;
      ...
      this.$postData(
        this.$api.updateStatusBatchSupplier_supplierSetManage,
        {
          ids: unitId,
          disabled: updateType
        },
  ...
  handleTableRowButton(row, btns) {
    let formGroupList;
    if (btns.type === "edit") {
      this.getSingleDetailInfo(row.fpmpBasSupplierId).then(data => {
    ...
    // 删除
    if (btns.type === "remove") {
      this.$confirm("确定删除当前供应商吗?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        this.$postData(
          this.$api.removeSupplier_supplierSetManage,
          {
            fpmpBasSupplierId: row.fpmpBasSupplierId
          },
          true
        ).then(() => {
          this.$message.success("您已成功删除该供应商!");
          this.initPageData();
        });
      });
    } // 启用禁用
    if (btns.type === "open" || btns.type === "closed") {
      ...
      this.$confirm(`您确定要${quote}该供应商信息？`, "确认", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
  ...
  getSingleDetailInfo(id) {
    return new Promise(resolve => {
      this.$getData(this.$api.loadSupplierDetail_supplierSetManage, {
        id: id
      }).then(data => {
        resolve(data.data);
      });
