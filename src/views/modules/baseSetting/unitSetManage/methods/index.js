const methods = {
  ...
  handleOperateButton(data) {
    let formGroupList;
    if (data.flag === "add") {
      this.dialogFormSettings.dialogFormTitle = "新增计量单位信息";
      // 加载表单组
    ...
    if (data.flag === "delete") {
      if (!this.chosedUnitData.length) {
        this.$message.warning("请选择需删除的计量单位信息!");
        return;
      }
      let unitId = this.chosedUnitData.map(item => {
        return item.fpmpBasUnitId;
      });
      this.$confirm("您确定要删除选中的计量单位信息？", "确认", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        this.$postData(
          this.$api.removeBatchUnit_unitSetManage,
          {
            ids: unitId
          },
          true
    ...
    if (data.flag === "open" || data.flag === "closed") {
      ...
      if (!this.chosedUnitData.length) {
        this.$message.warning(`请选择需${quote}的计量单位信息!`);
        return;
      }
      ...
      if ([...new Set(status)].length > 1) {
        this.$message.warning("请选择相同状态的计量单位信息!");
        return;
      }
      let unitId = this.chosedUnitData.map(item => {
        return item.fpmpBasUnitId;
      ...
      this.$postData(
        this.$api.updateStatusBatchUnit_unitSetManage,
        {
          ids: unitId,
          disabled: updateType
        },
  ...
  handleTableRowButton(row, btns) {
    let formGroupList;
    if (btns.type === "edit") {
      this.getSingleDetailInfo(row.fpmpBasUnitId).then(data => {
    ...
    // 删除
    if (btns.type === "remove") {
      this.$confirm("确定删除当前计量单位吗?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        this.$postData(
          this.$api.removeUnit_unitSetManage,
          {
            fpmpBasUnitId: row.fpmpBasUnitId,
            name: row.name
          },
          true
        ).then(() => {
          this.$message.success("您已成功删除该计量单位!");
          this.initPageData();
        });
      });
    }
    ...
    if (btns.type === "open" || btns.type === "closed") {
      ...
      this.$confirm(`您确定要${quote}该计量单位信息？`, "确认", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
  ...
  getSingleDetailInfo(id) {
    return new Promise(resolve => {
      this.$getData(this.$api.loadUnitDetail_unitSetManage, {
        id: id
      }).then(data => {
        resolve(data.data);
      });
